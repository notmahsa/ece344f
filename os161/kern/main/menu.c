...
cmd_dbflagsmenu(int n, char **a) {
    ...
    int dbflagCopy = DBFLAG;
    char result[3];
    result[2] = (dbflagCopy % 16 > 10 ? 'A' + dbflagCopy % 16 : dbflagCopy % 16 + '0');
    dbflagCopy /= 16;
    result[1] = (dbflagCopy % 16 > 10 ? 'A' + dbflagCopy % 16 : dbflagCopy % 16 + '0');
    dbflagCopy /= 16;
    result[0] = (dbflagCopy % 16 > 10 ? 'A' + dbflagCopy % 16 : dbflagCopy % 16 + '0');
    result[0] = dbflagCopy % 16 == 0 ? NULL : (dbflagCopy % 16 > 10 ? 'A' + dbflagCopy % 16 : dbflagCopy % 16 + '0');
    if (result[0] == NULL){
    	if (result[1] == '0')
    		result[1] = NULL;
    }
    kprintf("Current value of dbflags is 0x%c%c%c\n", result[0], result[1], result[2]);
    return 0;
}
...
static int cmd_df5o(int nargs, char **args) {

    int bit = atoi(args[1]);
    bit = bit - 1;

    if (nargs != 3 || (strcmp(args[2], "on") && strcmp(args[2], "off")) || bit > 11 || bit < 0) {
